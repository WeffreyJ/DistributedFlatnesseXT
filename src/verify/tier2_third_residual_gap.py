"""Tier-2 nominal-vs-plant diagnostic for the third residual family."""
...
def _tier2_edge_band_bias_cfg(config_path: str):
    ...
    cfg_t2.system.tier2.residual.mode = "edge_band_bias"
...
def run_tier2_third_residual_gap(config_path: str = "configs/system.yaml") -> Path:
    cfg = _tier2_edge_band_bias_cfg(config_path)
    out_dir = make_results_dir("tier2_third_residual_gap")
    sim = simulate_closed_loop(
        cfg,
        x0=_base_x0(cfg),
        horizon=float(getattr(cfg.gate3, "run_horizon", cfg.system.horizon)),
        options=SimOptions(blending_on=True, noise_delta=0.0, seed=int(cfg.seed) + 20720),
    )

    rows = _collect_rows(sim, cfg)
    _write_csv(out_dir / "tier2_third_residual_gap_table.csv", rows)
    dump_json(out_dir / "tier2_third_residual_gap_summary.json", _summary_payload(rows, cfg))
    ...
    _hist_plot(
        ...
        title="Tier-2 third residual norm histogram",
    ...
    _scatter_plot(
        ...
        title="Tier-2 third residual plant-vs-nominal mismatch",
    ...
    _scatter_plot(
        ...
        ylabel="||r(pi)-r(pi')||",
        title="Tier-2 third residual order-gap comparison",
        out_path=out_dir / "order_gap_nominal_vs_plant.png",
        diagonal=True,
    )
    return out_dir / "tier2_third_residual_gap_summary.json"


def main() -> None:
    parser = argparse.ArgumentParser(description="Tier-2 third residual nominal-vs-plant gap.")
    parser.add_argument("--config", default="configs/system.yaml")
    args = parser.parse_args()
    out = run_tier2_third_residual_gap(config_path=args.config)
    print(f"Wrote {out}")
