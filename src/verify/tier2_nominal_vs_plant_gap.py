"""Tier-2 nominal-vs-plant reference diagnostic under a small structured residual."""
...
def _tier2_residual_cfg(config_path: str):
...
def _scatter_plot(xvals: np.ndarray, yvals: np.ndarray, *, xlabel: str, ylabel: str, title: str, out_path: Path, diagonal: bool = False) -> None:
...
def _summary_payload(rows: list[dict[str, object]], cfg) -> dict[str, object]:
    residual_norms = np.asarray(
        [float(r["residual_norm_old"]) for r in rows] + [float(r["residual_norm_new"]) for r in rows],
        dtype=float,
    ) if rows else np.array([], dtype=float)
    nominal_vs_plant = np.asarray(
        [float(r["nominal_vs_plant_gap_old"]) for r in rows] + [float(r["nominal_vs_plant_gap_new"]) for r in rows],
        dtype=float,
    ) if rows else np.array([], dtype=float)
    ...
    plant_order_gap = np.asarray([float(r["plant_order_gap"]) for r in rows], dtype=float) if rows else np.array([], dtype=float)
    ...
    return {
        ...
        "mean_plant_order_gap": float(np.mean(plant_order_gap)) if plant_order_gap.size else 0.0,
        "max_plant_order_gap": float(np.max(plant_order_gap)) if plant_order_gap.size else 0.0,
...
def run_tier2_nominal_vs_plant_gap(config_path: str = "configs/system.yaml") -> Path:
    cfg = _tier2_residual_cfg(config_path)
    out_dir = make_results_dir("tier2_nominal_vs_plant_gap")
    sim = simulate_closed_loop(
        cfg,
        x0=_base_x0(cfg),
        horizon=float(getattr(cfg.gate3, "run_horizon", cfg.system.horizon)),
        options=SimOptions(blending_on=True, noise_delta=0.0, seed=int(cfg.seed) + 20700),
    )

    rows = _collect_rows(sim, cfg)
    table_path = out_dir / "tier2_nominal_vs_plant_gap_table.csv"
    summary_path = out_dir / "tier2_nominal_vs_plant_gap_summary.json"
    _write_csv(table_path, rows)
    dump_json(summary_path, _summary_payload(rows, cfg))
    ...
    _hist_plot(
        ...
        title="Tier-2 residual norm histogram",
    ...
    _scatter_plot(
        ...
        title="Tier-2 plant-vs-nominal mismatch",
    ...
    _scatter_plot(
        ...
        ylabel="||Delta_plant(pi') - Delta_plant(pi)||",
        title="Tier-2 nominal vs plant order gap",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Tier-2 nominal-vs-plant gap diagnostic")
    parser.add_argument("--config", default="configs/system.yaml")
    args = parser.parse_args()
    path = run_tier2_nominal_vs_plant_gap(args.config)
    print(f"Wrote {path}")
