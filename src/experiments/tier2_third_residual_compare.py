"""Frozen-controller comparison for Tier-1 nominal vs Tier-2 edge-band-bias residual."""
...
def _clone_cfg_variant(cfg, variant: str):
    ...
    elif variant == "tier2_edge_band_bias_enabled":
        cfg_mode.system.plant_family = "tier2"
        cfg_mode.system.tier2.residual.enabled = True
        cfg_mode.system.tier2.residual.mode = "edge_band_bias"
...
def run_tier2_third_residual_compare(
    config_path: str = "configs/system.yaml",
    out_dir: str | Path = "results/tier2_third_residual_compare",
) -> Path:
    cfg = load_config(config_path)
    out = make_results_dir("tier2_third_residual_compare") if out_dir == "results/tier2_third_residual_compare" else Path(out_dir)
    out.mkdir(parents=True, exist_ok=True)

    x0 = _base_x0(cfg)
    sim_seed = int(cfg.seed) + 20841
    options = SimOptions(
        blending_on=True,
        noise_delta=0.0,
        seed=sim_seed,
    ...
    variants = ["tier1_nominal", "tier2_edge_band_bias_enabled"]
    ...
    summary = {
        ...
        "delta_tier2_edge_band_bias_minus_tier1": {
    ...
    dump_json(out / "tier2_third_residual_compare.json", summary)
    _write_csv(out / "tier2_third_residual_compare.csv", rows)

    _plot_grouped_bars(
        rows,
        ["switch_count", "transition_start_count", "blend_active_steps"],
        "Tier-2 third residual comparison: switching burden",
        "count",
    ...
    _plot_grouped_bars(
        ...
        "Tier-2 third residual comparison: tracking error",
        "tracking error",
    ...
    _plot_grouped_bars(
        ...
        "Tier-2 third residual comparison: jump magnitudes",
        "jump norm",
    ...
    _plot_grouped_bars(
        ...
        "Tier-2 third residual comparison: nominal-vs-plant mismatch",
        "mismatch norm",
        out / "mismatch_compare.png",
    )
    return out / "tier2_third_residual_compare.json"


def main() -> None:
    parser = argparse.ArgumentParser(description="Compare Tier-1 nominal and Tier-2 edge-band-bias residual.")
    parser.add_argument("--config", default="configs/system.yaml")
    parser.add_argument("--out", default="results/tier2_third_residual_compare")
    args = parser.parse_args()
    out = run_tier2_third_residual_compare(config_path=args.config, out_dir=args.out)
    print(f"Wrote {out}")
