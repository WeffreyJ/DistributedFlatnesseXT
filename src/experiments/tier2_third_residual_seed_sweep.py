"""Paired seed sweep for Tier-1 nominal vs Tier-2 edge-band-bias residual."""
...
def run_tier2_third_residual_seed_sweep(
    config_path: str = "configs/system.yaml",
    out_dir: str | Path = "results/tier2_third_residual_seed_sweep",
    seed_ids: list[int] | None = None,
) -> Path:
    cfg = load_config(config_path)
    out = (
        make_results_dir("tier2_third_residual_seed_sweep")
        if out_dir == "results/tier2_third_residual_seed_sweep"
        else Path(out_dir)
    )
    ...
    for seed_id in seed_list:
        x0_seed = int(cfg.seed) + 20961 + 10 * seed_id
        sim_seed = int(cfg.seed) + 20981 + 10 * seed_id
        ...
        delta = _paired_delta(seed_id, seed_runs["tier1_nominal"], seed_runs["tier2_edge_band_bias_enabled"])
        paired_rows.append(
            {
                "seed_id": int(seed_id),
                "tier1_switch_count": int(seed_runs["tier1_nominal"]["switch_count"]),
                "tier2_switch_count": int(seed_runs["tier2_edge_band_bias_enabled"]["switch_count"]),
                ...
                "tier2_transition_start_count": int(seed_runs["tier2_edge_band_bias_enabled"]["transition_start_count"]),
                ...
                "tier2_blend_active_steps": int(seed_runs["tier2_edge_band_bias_enabled"]["blend_active_steps"]),
                ...
                "tier2_tracking_error_mean": float(seed_runs["tier2_edge_band_bias_enabled"]["tracking_error_mean"]),
                ...
                "tier2_tracking_error_max": float(seed_runs["tier2_edge_band_bias_enabled"]["tracking_error_max"]),
                ...
                "tier2_max_raw_jump": float(seed_runs["tier2_edge_band_bias_enabled"]["max_raw_jump"]),
                "delta_max_raw_jump": float(delta["delta_max_raw_jump"]),
                "tier1_max_applied_jump": float(seed_runs["tier1_nominal"]["max_applied_jump"]),
                "tier2_max_applied_jump": float(seed_runs["tier2_edge_band_bias_enabled"]["max_applied_jump"]),
                ...
                "tier2_mean_nominal_vs_plant_gap": float(seed_runs["tier2_edge_band_bias_enabled"]["mean_nominal_vs_plant_gap_over_rollout"]),
                ...
                "tier2_max_nominal_vs_plant_gap": float(seed_runs["tier2_edge_band_bias_enabled"]["max_nominal_vs_plant_gap_over_rollout"]),
                ...
                "tier2_fraction_steps_with_nonzero_mismatch": float(seed_runs["tier2_edge_band_bias_enabled"]["fraction_steps_with_nonzero_mismatch"]),
                ...
                "tier2_mismatch_at_switch_steps_mean": float(seed_runs["tier2_edge_band_bias_enabled"]["mismatch_at_switch_steps_mean"]),
        ...
        runs_by_seed[str(seed_id)] = {
            ...
            "tier2_edge_band_bias_enabled": seed_runs["tier2_edge_band_bias_enabled"],
    ...
    dump_json(out / "tier2_third_residual_seed_sweep.json", summary)
    _write_csv(out / "tier2_third_residual_seed_sweep.csv", paired_rows)

    _hist_plot(
        np.asarray([float(row["delta_switch_count"]) for row in paired_rows], dtype=float),
        xlabel="delta_switch_count",
        title="Tier-2 edge-band-bias minus Tier-1 switch-count deltas",
    ...
    _hist_plot(
        ...
        title="Tier-2 edge-band-bias minus Tier-1 tracking-error-mean deltas",
    ...
    _hist_plot(
        ...
        title="Tier-2 edge-band-bias minus Tier-1 applied-jump deltas",
    ...
    _hist_plot(
        ...
        title="Tier-2 edge-band-bias minus Tier-1 mean mismatch deltas",
    ...
    _scatter_plot(
        ...
        title="Tier-2 edge-band-bias mismatch-at-switch vs switch-count deltas",
        out_path=out / "delta_mismatch_vs_switch_scatter.png",
    )
    return out / "tier2_third_residual_seed_sweep.json"


def main() -> None:
    parser = argparse.ArgumentParser(description="Paired seed sweep for Tier-2 edge-band-bias residual.")
    parser.add_argument("--config", default="configs/system.yaml")
    parser.add_argument("--out", default="results/tier2_third_residual_seed_sweep")
    parser.add_argument("--seed-ids", nargs="*", type=int, default=None)
    args = parser.parse_args()
    out = run_tier2_third_residual_seed_sweep(
