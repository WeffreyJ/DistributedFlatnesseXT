"""Designed mismatch-sensitive scenarios for the Tier-2 longitudinal-bias residual."""
...
def _case_definitions(cfg) -> list[dict[str, object]]:
    ...
    bias_center = float(cfg.system.tier2.residual.longitudinal_bias_center)
    ds_target = gamma_edge + bias_center
    return [
        {
            "case_name": "longitudinal_band_dwell_centered",
            "family": "longitudinal_band_dwell",
            "x0": [ds_target, 0.0, -ds_target, -0.004, 0.002, 0.002],
            "tier1_lateral_offsets": [0.0, 0.8, -0.8],
        },
        {
            "case_name": "longitudinal_band_slow_crossing",
            "family": "longitudinal_band_dwell",
            "x0": [ds_target - 0.01, 0.0, -0.21, -0.006, 0.003, 0.003],
            "tier1_lateral_offsets": [0.0, 1.2, -0.7],
        },
        {
            "case_name": "near_switch_longitudinal_bias",
            "family": "near_switch_longitudinal_amplification",
            "x0": [0.095, 0.091, -0.205, -0.004, 0.004, 0.0],
            "tier1_lateral_offsets": [0.0, 1.1, -0.9],
        },
        {
            "case_name": "near_switch_longitudinal_opposed",
            "family": "near_switch_longitudinal_amplification",
            "x0": [0.094, 0.0905, -0.204, -0.005, 0.005, 0.0],
            "tier1_lateral_offsets": [0.0, 1.9, -1.7],
        },
        {
            "case_name": "mixed_boundary_longitudinal_aligned",
            "family": "mixed_boundary_longitudinal_alignment",
            "x0": [ds_target, 0.0, -0.21, -0.006, 0.006, 0.0],
            "tier1_lateral_offsets": [0.0, 2.45, -1.10],
        },
        {
            "case_name": "mixed_boundary_band_occupancy_split",
            "family": "mixed_boundary_longitudinal_alignment",
            "x0": [ds_target + 0.01, 0.0, -0.22, -0.008, 0.004, 0.004],
            "tier1_lateral_offsets": [0.0, 2.0, -0.6],
        },
        {
            "case_name": "asymmetric_band_occupancy",
            "family": "asymmetric_band_occupancy",
            "x0": [ds_target, 0.05, -0.19, -0.004, 0.001, 0.003],
            "tier1_lateral_offsets": [0.0, 0.65, -2.2],
        },
    ]
...
def run_tier2_second_residual_scenarios(
    config_path: str = "configs/system.yaml",
    out_dir: str | Path = "results/tier2_second_residual_scenarios",
) -> Path:
    cfg = load_config(config_path)
    out = (
        make_results_dir("tier2_second_residual_scenarios")
        if out_dir == "results/tier2_second_residual_scenarios"
        else Path(out_dir)
    )
    out.mkdir(parents=True, exist_ok=True)

    sim_seed = int(cfg.seed) + 21131
    options = SimOptions(
        blending_on=True,
        noise_delta=0.0,
        seed=sim_seed,
    ...
    for case in _case_definitions(cfg):
        ...
        switch_pattern_differs = list(raw_sims["tier1_nominal"].get("switch_steps", [])) != list(
            raw_sims["tier2_longitudinal_bias_enabled"].get("switch_steps", [])
        )
        candidate_pattern_differs = list(raw_sims["tier1_nominal"].get("pi_candidate", [])) != list(
            raw_sims["tier2_longitudinal_bias_enabled"].get("pi_candidate", [])
        )
        effective_candidate_pattern_differs = list(raw_sims["tier1_nominal"].get("effective_pi_candidate", [])) != list(
            raw_sims["tier2_longitudinal_bias_enabled"].get("effective_pi_candidate", [])
        )
        delta = _case_delta(
            case_name,
            runs["tier1_nominal"],
            runs["tier2_longitudinal_bias_enabled"],
        ...
        csv_rows.append(
            {
                ...
                **delta,
                ...
                "tier2_switch_count": int(runs["tier2_longitudinal_bias_enabled"]["switch_count"]),
                "tier1_transition_start_count": int(runs["tier1_nominal"]["transition_start_count"]),
                "tier2_transition_start_count": int(runs["tier2_longitudinal_bias_enabled"]["transition_start_count"]),
                "tier1_blend_active_steps": int(runs["tier1_nominal"]["blend_active_steps"]),
                "tier2_blend_active_steps": int(runs["tier2_longitudinal_bias_enabled"]["blend_active_steps"]),
                "tier1_tracking_error_mean": float(runs["tier1_nominal"]["tracking_error_mean"]),
                "tier2_tracking_error_mean": float(runs["tier2_longitudinal_bias_enabled"]["tracking_error_mean"]),
                "tier1_tracking_error_max": float(runs["tier1_nominal"]["tracking_error_max"]),
                "tier2_tracking_error_max": float(runs["tier2_longitudinal_bias_enabled"]["tracking_error_max"]),
                "tier1_max_raw_jump": float(runs["tier1_nominal"]["max_raw_jump"]),
                "tier2_max_raw_jump": float(runs["tier2_longitudinal_bias_enabled"]["max_raw_jump"]),
                "tier1_max_applied_jump": float(runs["tier1_nominal"]["max_applied_jump"]),
                "tier2_max_applied_jump": float(runs["tier2_longitudinal_bias_enabled"]["max_applied_jump"]),
                "tier1_mean_nominal_vs_plant_gap_over_rollout": float(runs["tier1_nominal"]["mean_nominal_vs_plant_gap_over_rollout"]),
                "tier2_mean_nominal_vs_plant_gap_over_rollout": float(runs["tier2_longitudinal_bias_enabled"]["mean_nominal_vs_plant_gap_over_rollout"]),
                "tier1_max_nominal_vs_plant_gap_over_rollout": float(runs["tier1_nominal"]["max_nominal_vs_plant_gap_over_rollout"]),
                "tier2_max_nominal_vs_plant_gap_over_rollout": float(runs["tier2_longitudinal_bias_enabled"]["max_nominal_vs_plant_gap_over_rollout"]),
                "tier1_mismatch_at_switch_steps_mean": float(runs["tier1_nominal"]["mismatch_at_switch_steps_mean"]),
                "tier2_mismatch_at_switch_steps_mean": float(runs["tier2_longitudinal_bias_enabled"]["mismatch_at_switch_steps_mean"]),
            }
        )
        summary_cases[case_name] = {
            ...
            "tier2_longitudinal_bias_enabled": runs["tier2_longitudinal_bias_enabled"],
    ...
    dump_json(out / "tier2_second_residual_scenarios.json", summary)
    _write_csv(out / "tier2_second_residual_scenarios.csv", csv_rows)

    _bar_plot(
        csv_rows,
        "delta_switch_count",
        "Tier-2 longitudinal-bias minus Tier-1 switch-count delta by case",
        "delta switch count",
    ...
    _bar_plot(
        csv_rows,
        "delta_max_applied_jump",
        "Tier-2 longitudinal-bias minus Tier-1 applied-jump delta by case",
    ...
    _bar_plot(
        csv_rows,
        "delta_tracking_error_mean",
        "Tier-2 longitudinal-bias minus Tier-1 tracking-error-mean delta by case",
    ...
    _bar_plot(
        csv_rows,
        "delta_mean_nominal_vs_plant_gap_over_rollout",
        "Tier-2 longitudinal-bias minus Tier-1 mean mismatch delta by case",
        "delta mean mismatch",
        out / "mismatch_delta_by_case.png",
    )

    return out / "tier2_second_residual_scenarios.json"


def main() -> None:
    parser = argparse.ArgumentParser(description="Designed scenarios for the Tier-2 longitudinal-bias residual.")
    parser.add_argument("--config", default="configs/system.yaml")
    parser.add_argument("--out", default="results/tier2_second_residual_scenarios")
    args = parser.parse_args()
    out = run_tier2_second_residual_scenarios(config_path=args.config, out_dir=args.out)
    print(f"Wrote {out}")
