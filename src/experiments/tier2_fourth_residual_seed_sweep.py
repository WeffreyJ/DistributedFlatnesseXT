"""Paired seed sweep for Tier-1 nominal vs Tier-2 support-transition-bias residual."""
...
from src.experiments.tier2_fourth_residual_compare import _clone_cfg_variant, _mismatch_metrics, _blocked_steps_total
...
def _metrics_for_run(sim: dict[str, object], cfg, variant: str, seed_id: int) -> dict[str, float | int | str]:
    ...
    out = {
        ...
        "blocked_steps_total": int(_blocked_steps_total(sim)),
...
def _paired_delta(seed_id: int, tier1: dict[str, object], tier2: dict[str, object]) -> dict[str, float | int]:
    return {
        ...
        "delta_blocked_steps_total": int(tier2["blocked_steps_total"]) - int(tier1["blocked_steps_total"]),
        ...
        "delta_fraction_above_mismatch_tol": float(tier2["fraction_above_mismatch_tol"])
        - float(tier1["fraction_above_mismatch_tol"]),
...
def _write_csv(path: Path, rows: list[dict[str, object]]) -> None:
    fieldnames = [
        ...
        "tier1_fraction_above_mismatch_tol",
        "tier2_fraction_above_mismatch_tol",
        "delta_fraction_above_mismatch_tol",
...
def run_tier2_fourth_residual_seed_sweep(
    config_path: str = "configs/system.yaml",
    out_dir: str | Path = "results/tier2_fourth_residual_seed_sweep",
    seed_ids: list[int] | None = None,
) -> Path:
    cfg = load_config(config_path)
    out = (
        make_results_dir("tier2_fourth_residual_seed_sweep")
        if out_dir == "results/tier2_fourth_residual_seed_sweep"
        else Path(out_dir)
    )
    ...
    for seed_id in seed_list:
        x0_seed = int(cfg.seed) + 21061 + 10 * seed_id
        sim_seed = int(cfg.seed) + 21081 + 10 * seed_id
        ...
        delta = _paired_delta(seed_id, seed_runs["tier1_nominal"], seed_runs["tier2_support_transition_bias_enabled"])
        paired_rows.append(
            {
                "seed_id": int(seed_id),
                "tier1_switch_count": int(seed_runs["tier1_nominal"]["switch_count"]),
                "tier2_switch_count": int(seed_runs["tier2_support_transition_bias_enabled"]["switch_count"]),
                ...
                "tier2_transition_start_count": int(seed_runs["tier2_support_transition_bias_enabled"]["transition_start_count"]),
                ...
                "tier2_blend_active_steps": int(seed_runs["tier2_support_transition_bias_enabled"]["blend_active_steps"]),
                "delta_blend_active_steps": int(delta["delta_blend_active_steps"]),
                "tier1_blocked_steps_total": int(seed_runs["tier1_nominal"]["blocked_steps_total"]),
                "tier2_blocked_steps_total": int(seed_runs["tier2_support_transition_bias_enabled"]["blocked_steps_total"]),
                ...
                "tier2_tracking_error_mean": float(seed_runs["tier2_support_transition_bias_enabled"]["tracking_error_mean"]),
                ...
                "tier2_tracking_error_max": float(seed_runs["tier2_support_transition_bias_enabled"]["tracking_error_max"]),
                ...
                "tier2_max_raw_jump": float(seed_runs["tier2_support_transition_bias_enabled"]["max_raw_jump"]),
                "delta_max_raw_jump": float(delta["delta_max_raw_jump"]),
                "tier1_max_applied_jump": float(seed_runs["tier1_nominal"]["max_applied_jump"]),
                "tier2_max_applied_jump": float(seed_runs["tier2_support_transition_bias_enabled"]["max_applied_jump"]),
                ...
                "tier2_mean_nominal_vs_plant_gap": float(seed_runs["tier2_support_transition_bias_enabled"]["mean_nominal_vs_plant_gap_over_rollout"]),
                ...
                "tier2_max_nominal_vs_plant_gap": float(seed_runs["tier2_support_transition_bias_enabled"]["max_nominal_vs_plant_gap_over_rollout"]),
                ...
                "tier2_fraction_steps_with_nonzero_mismatch": float(seed_runs["tier2_support_transition_bias_enabled"]["fraction_steps_with_nonzero_mismatch"]),
                "delta_fraction_steps_with_nonzero_mismatch": float(delta["delta_fraction_steps_with_nonzero_mismatch"]),
                "tier1_fraction_above_mismatch_tol": float(seed_runs["tier1_nominal"]["fraction_above_mismatch_tol"]),
                "tier2_fraction_above_mismatch_tol": float(seed_runs["tier2_support_transition_bias_enabled"]["fraction_above_mismatch_tol"]),
                "delta_fraction_above_mismatch_tol": float(delta["delta_fraction_above_mismatch_tol"]),
                "tier1_mismatch_at_switch_steps_mean": float(seed_runs["tier1_nominal"]["mismatch_at_switch_steps_mean"]),
                "tier2_mismatch_at_switch_steps_mean": float(seed_runs["tier2_support_transition_bias_enabled"]["mismatch_at_switch_steps_mean"]),
        ...
        runs_by_seed[str(seed_id)] = {
            ...
            "tier2_support_transition_bias_enabled": seed_runs["tier2_support_transition_bias_enabled"],
    ...
    dump_json(out / "tier2_fourth_residual_seed_sweep.json", summary)
    _write_csv(out / "tier2_fourth_residual_seed_sweep.csv", paired_rows)

    _hist_plot(
        np.asarray([float(row["delta_switch_count"]) for row in paired_rows], dtype=float),
        xlabel="delta switch_count",
        title="Tier-2 fourth residual: delta switch count",
        out_path=out / "delta_switches_hist.png",
    )
    _hist_plot(
        np.asarray([float(row["delta_tracking_error_mean"]) for row in paired_rows], dtype=float),
        xlabel="delta tracking_error_mean",
        title="Tier-2 fourth residual: delta tracking error mean",
        out_path=out / "delta_tracking_hist.png",
    )
    _hist_plot(
        np.asarray([float(row["delta_max_applied_jump"]) for row in paired_rows], dtype=float),
        xlabel="delta max_applied_jump",
        title="Tier-2 fourth residual: delta applied jump",
        out_path=out / "delta_jumps_hist.png",
    )
    _hist_plot(
        np.asarray([float(row["delta_mean_nominal_vs_plant_gap"]) for row in paired_rows], dtype=float),
        xlabel="delta mean nominal-vs-plant gap",
        title="Tier-2 fourth residual: delta mismatch mean",
    ...
    _scatter_plot(
        ...
        np.asarray([float(row["delta_switch_count"]) for row in paired_rows], dtype=float),
        xlabel="delta mismatch at switch steps mean",
        ylabel="delta switch_count",
        title="Tier-2 fourth residual: mismatch-vs-switch delta",
        out_path=out / "delta_mismatch_vs_switch_scatter.png",
    )
    return out / "tier2_fourth_residual_seed_sweep.json"


def main() -> None:
    parser = argparse.ArgumentParser(description="Seed sweep for Tier-1 nominal vs Tier-2 support-transition-bias residual.")
    parser.add_argument("--config", default="configs/system.yaml")
    parser.add_argument("--out", default="results/tier2_fourth_residual_seed_sweep")
    parser.add_argument("--seed-ids", nargs="*", type=int)
    args = parser.parse_args()
    out = run_tier2_fourth_residual_seed_sweep(config_path=args.config, out_dir=args.out, seed_ids=args.seed_ids)
    print(f"Wrote {out}")
