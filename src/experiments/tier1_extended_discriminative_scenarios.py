"""Extended designed Tier-1 scenario families under frozen controller logic."""
...
def _case_definitions(cfg) -> list[dict[str, object]]:
    ...
    return [
        {
            "case_name": "longitudinal_gamma_just_above",
            "family": "longitudinal_boundary",
            "x0": [gamma_edge + 0.004, 0.0, -0.20, 0.0, 0.0, 0.0],
        },
        {
            "case_name": "longitudinal_gamma_just_below",
            "family": "longitudinal_boundary",
            "x0": [gamma_edge - 0.004, 0.0, -0.20, 0.0, 0.0, 0.0],
        },
        {
...
def _metrics_for_run(sim: dict[str, object], *, case_name: str, plant_family: str) -> dict[str, object]:
...
def _case_delta(
    ...
    *,
    ...
    active_edge_pattern_differs: bool,
) -> dict[str, object]:
    return {
        "case_name": str(case_name),
...
def run_tier1_extended_discriminative_scenarios(
    config_path: str = "configs/system.yaml",
    out_dir: str | Path = "results/tier1_extended_discriminative_scenarios",
) -> Path:
    cfg = load_config(config_path)
    out = (
        make_results_dir("tier1_extended_discriminative_scenarios")
        if out_dir == "results/tier1_extended_discriminative_scenarios"
        else Path(out_dir)
    )
    out.mkdir(parents=True, exist_ok=True)

    sim_seed = int(cfg.seed) + 19921
    options = SimOptions(
        blending_on=True,
        noise_delta=0.0,
        seed=sim_seed,
    ...
    for case in cases:
        ...
        for plant_family in ["tier0", "tier1"]:
            ...
            runs[plant_family] = _metrics_for_run(sim, case_name=case_name, plant_family=plant_family)
    ...
    dump_json(out / "tier1_extended_discriminative_scenarios.json", summary)
    _write_csv(out / "tier1_extended_discriminative_scenarios.csv", csv_rows)
    ...
    return out / "tier1_extended_discriminative_scenarios.json"


def main() -> None:
    parser = argparse.ArgumentParser(description="Extended designed Tier-1 discriminative scenarios.")
    parser.add_argument("--config", default="configs/system.yaml")
    parser.add_argument("--out", default="results/tier1_extended_discriminative_scenarios")
    args = parser.parse_args()
    out = run_tier1_extended_discriminative_scenarios(
