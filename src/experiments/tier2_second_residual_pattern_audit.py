"""Mechanism audit for Tier-2 longitudinal-bias pattern changes."""
...
from src.experiments.tier2_second_residual_compare import _clone_cfg_variant
from src.experiments.tier2_second_residual_scenarios import _case_definitions
...
def _longitudinal_band_distance(x: np.ndarray, cfg) -> float:
    center = float(cfg.system.tier2.residual.longitudinal_bias_center)
    gamma_edge = float(cfg.system.gamma_edge)
    edges = active_edges_tier1(np.asarray(x, dtype=float), cfg.system)
    if not edges:
        return float("inf")
    distances = []
    for leader, follower in edges:
        geom = pairwise_geometry(np.asarray(x, dtype=float), leader, follower, cfg.system)
        ds_eff = max(float(geom["ds"]) - gamma_edge, 0.0)
        distances.append(abs(ds_eff - center))
...
def _step_rows_for_case(case: dict[str, object], cfg, options: SimOptions) -> tuple[list[dict[str, object]], dict[str, object]]:
    ...
    cfg_t2 = _clone_cfg_variant(cfg, "tier2_longitudinal_bias_enabled")
    ...
    band_distance = np.array(
        [_longitudinal_band_distance(x_hist_t2[k], cfg_t2) for k in range(max(len(x_hist_t2) - 1, 0))],
        dtype=float,
    )
        ...
        rows.append(
            {
                ...
                "longitudinal_band_distance": float(band_distance[k]),
    ...
    summary = {
        ...
        "mean_longitudinal_band_distance_at_pattern_difference_steps": float(
            np.mean([row["longitudinal_band_distance"] for row in rows])
        )
        if rows
        else float("inf"),
...
def run_tier2_second_residual_pattern_audit(
    config_path: str = "configs/system.yaml",
    out_dir: str | Path = "results/tier2_second_residual_pattern_audit",
) -> Path:
    cfg = load_config(config_path)
    out = (
        make_results_dir("tier2_second_residual_pattern_audit")
        if out_dir == "results/tier2_second_residual_pattern_audit"
        else Path(out_dir)
    )
    out.mkdir(parents=True, exist_ok=True)

    sim_seed = int(cfg.seed) + 21231
    options = SimOptions(
        blending_on=True,
        noise_delta=0.0,
        seed=sim_seed,
    ...
    summary = {
        ...
        "mean_longitudinal_band_distance_at_pattern_difference_steps": {
            case_name: float(case_summary["mean_longitudinal_band_distance_at_pattern_difference_steps"])
            for case_name, case_summary in case_summaries.items()
        },
        ...
        "mean_longitudinal_band_distance_at_pattern_difference_steps_global": (
            float(np.mean([float(row["longitudinal_band_distance"]) for row in all_rows])) if all_rows else float("inf")
        ),
    }

    dump_json(out / "tier2_second_residual_pattern_audit.json", summary)
    _write_csv(out / "tier2_second_residual_pattern_audit.csv", all_rows)

    case_names = list(case_summaries.keys())
    _bar_plot(
        case_names,
        [float(case_summaries[name]["total_pattern_difference_steps"]) for name in case_names],
        "Tier-2 longitudinal-bias: pattern difference steps by case",
    ...
    _bar_plot(
        ...
        "Tier-2 longitudinal-bias: mismatch at pattern differences",
        "mismatch norm",
    ...
    _bar_plot(
        ...
        "Tier-2 longitudinal-bias: tie margin at pattern differences",
        "tie margin",
        out / "tie_margin_at_pattern_differences.png",
    )
    _bar_plot(
        case_names,
        [float(case_summaries[name]["mean_longitudinal_band_distance_at_pattern_difference_steps"]) for name in case_names],
        "Tier-2 longitudinal-bias: band distance at pattern differences",
        "distance to bias center",
        out / "longitudinal_band_distance_at_pattern_differences.png",
    )
    return out / "tier2_second_residual_pattern_audit.json"


def main() -> None:
    parser = argparse.ArgumentParser(description="Audit Tier-2 longitudinal-bias pattern changes.")
    parser.add_argument("--config", default="configs/system.yaml")
    parser.add_argument("--out", default="results/tier2_second_residual_pattern_audit")
    args = parser.parse_args()
    out = run_tier2_second_residual_pattern_audit(config_path=args.config, out_dir=args.out)
    print(f"Wrote {out}")
