"""Designed edge-band-sensitive scenarios for the Tier-2 edge-band-bias residual."""
...
from src.experiments.tier2_third_residual_compare import _clone_cfg_variant, _mismatch_metrics
...
def _case_definitions(cfg) -> list[dict[str, object]]:
    ...
    long_center = float(cfg.system.tier2.residual.edge_band_longitudinal_center)
    rho_center = float(cfg.system.tier2.residual.edge_band_transverse_center)
    ds_target = gamma_edge + long_center
    return [
        {
            "case_name": "edge_band_centered_dwell",
            "family": "edge_band_dwell",
            "x0": [ds_target, 0.0, -0.20, -0.003, 0.0015, 0.0015],
            "tier1_lateral_offsets": [0.0, rho_center, -rho_center],
        },
        {
            "case_name": "edge_band_slow_crossing",
            "family": "edge_band_dwell",
            "x0": [ds_target - 0.015, 0.0, -0.21, -0.006, 0.003, 0.003],
            "tier1_lateral_offsets": [0.0, rho_center - 0.10, -(rho_center - 0.10)],
        },
        {
            "case_name": "near_switch_edge_band_bias",
            "family": "near_switch_edge_band_amplification",
            "x0": [0.095, 0.091, -0.205, -0.004, 0.004, 0.0],
            "tier1_lateral_offsets": [0.0, rho_center - 0.20, -(rho_center - 0.40)],
        },
        {
            "case_name": "near_switch_edge_band_opposed",
            "family": "near_switch_edge_band_amplification",
            "x0": [0.094, 0.0905, -0.204, -0.005, 0.005, 0.0],
            "tier1_lateral_offsets": [0.0, rho_center + 0.10, -(rho_center - 0.15)],
        },
        {
            "case_name": "mixed_boundary_edge_band_aligned",
            "family": "mixed_boundary_edge_band_alignment",
            "x0": [ds_target, 0.0, -0.21, -0.006, 0.006, 0.0],
            "tier1_lateral_offsets": [0.0, rho_center, -1.10],
        },
        {
            "case_name": "mixed_boundary_edge_band_split",
            "family": "mixed_boundary_edge_band_alignment",
            "x0": [ds_target + 0.01, 0.0, -0.22, -0.008, 0.004, 0.004],
            "tier1_lateral_offsets": [0.0, rho_center + 0.05, -0.60],
        },
        {
            "case_name": "asymmetric_edge_band_occupancy",
            "family": "asymmetric_edge_band_occupancy",
            "x0": [ds_target, 0.05, -0.19, -0.004, 0.001, 0.003],
            "tier1_lateral_offsets": [0.0, rho_center - 0.30, -2.2],
        },
    ]
...
def run_tier2_third_residual_scenarios(
    config_path: str = "configs/system.yaml",
    out_dir: str | Path = "results/tier2_third_residual_scenarios",
) -> Path:
    cfg = load_config(config_path)
    out = (
        make_results_dir("tier2_third_residual_scenarios")
        if out_dir == "results/tier2_third_residual_scenarios"
        else Path(out_dir)
    )
    out.mkdir(parents=True, exist_ok=True)

    sim_seed = int(cfg.seed) + 21141
    options = SimOptions(
        blending_on=True,
        noise_delta=0.0,
        seed=sim_seed,
    ...
    for case in _case_definitions(cfg):
        ...
        for variant in ["tier1_nominal", "tier2_edge_band_bias_enabled"]:
        ...
        switch_pattern_differs = list(raw_sims["tier1_nominal"].get("switch_steps", [])) != list(
            raw_sims["tier2_edge_band_bias_enabled"].get("switch_steps", [])
        )
        candidate_pattern_differs = list(raw_sims["tier1_nominal"].get("pi_candidate", [])) != list(
            raw_sims["tier2_edge_band_bias_enabled"].get("pi_candidate", [])
        )
        effective_candidate_pattern_differs = list(raw_sims["tier1_nominal"].get("effective_pi_candidate", [])) != list(
            raw_sims["tier2_edge_band_bias_enabled"].get("effective_pi_candidate", [])
        )

        delta = _case_delta(
            case_name,
            runs["tier1_nominal"],
            runs["tier2_edge_band_bias_enabled"],
        ...
        csv_rows.append(
            {
                ...
                **delta,
                ...
                "tier2_switch_count": int(runs["tier2_edge_band_bias_enabled"]["switch_count"]),
                "tier1_transition_start_count": int(runs["tier1_nominal"]["transition_start_count"]),
                "tier2_transition_start_count": int(runs["tier2_edge_band_bias_enabled"]["transition_start_count"]),
                "tier1_blend_active_steps": int(runs["tier1_nominal"]["blend_active_steps"]),
                "tier2_blend_active_steps": int(runs["tier2_edge_band_bias_enabled"]["blend_active_steps"]),
                "tier1_tracking_error_mean": float(runs["tier1_nominal"]["tracking_error_mean"]),
                "tier2_tracking_error_mean": float(runs["tier2_edge_band_bias_enabled"]["tracking_error_mean"]),
                "tier1_tracking_error_max": float(runs["tier1_nominal"]["tracking_error_max"]),
                "tier2_tracking_error_max": float(runs["tier2_edge_band_bias_enabled"]["tracking_error_max"]),
                "tier1_max_raw_jump": float(runs["tier1_nominal"]["max_raw_jump"]),
                "tier2_max_raw_jump": float(runs["tier2_edge_band_bias_enabled"]["max_raw_jump"]),
                "tier1_max_applied_jump": float(runs["tier1_nominal"]["max_applied_jump"]),
                "tier2_max_applied_jump": float(runs["tier2_edge_band_bias_enabled"]["max_applied_jump"]),
                "tier1_mean_nominal_vs_plant_gap_over_rollout": float(runs["tier1_nominal"]["mean_nominal_vs_plant_gap_over_rollout"]),
                "tier2_mean_nominal_vs_plant_gap_over_rollout": float(runs["tier2_edge_band_bias_enabled"]["mean_nominal_vs_plant_gap_over_rollout"]),
                "tier1_max_nominal_vs_plant_gap_over_rollout": float(runs["tier1_nominal"]["max_nominal_vs_plant_gap_over_rollout"]),
                "tier2_max_nominal_vs_plant_gap_over_rollout": float(runs["tier2_edge_band_bias_enabled"]["max_nominal_vs_plant_gap_over_rollout"]),
                "tier1_mismatch_at_switch_steps_mean": float(runs["tier1_nominal"]["mismatch_at_switch_steps_mean"]),
                "tier2_mismatch_at_switch_steps_mean": float(runs["tier2_edge_band_bias_enabled"]["mismatch_at_switch_steps_mean"]),
            }
        )
        summary_cases[case_name] = {
            ...
            "tier2_edge_band_bias_enabled": runs["tier2_edge_band_bias_enabled"],
            "delta_tier2_edge_band_bias_minus_tier1": delta,
        }
    ...
    dump_json(out / "tier2_third_residual_scenarios.json", summary)
    _write_csv(out / "tier2_third_residual_scenarios.csv", csv_rows)

    _bar_plot(
        csv_rows,
        "delta_switch_count",
        "Tier-2 edge-band-bias minus Tier-1 switch-count delta by case",
        "delta switch count",
    ...
    _bar_plot(
        csv_rows,
        "delta_max_applied_jump",
        "Tier-2 edge-band-bias minus Tier-1 applied-jump delta by case",
    ...
    _bar_plot(
        csv_rows,
        "delta_tracking_error_mean",
        "Tier-2 edge-band-bias minus Tier-1 tracking-error-mean delta by case",
    ...
    _bar_plot(
        csv_rows,
        "delta_mean_nominal_vs_plant_gap_over_rollout",
        "Tier-2 edge-band-bias minus Tier-1 mean mismatch delta by case",
        "delta mean mismatch",
        out / "mismatch_delta_by_case.png",
    )

    return out / "tier2_third_residual_scenarios.json"


def main() -> None:
    parser = argparse.ArgumentParser(description="Designed edge-band-sensitive scenarios for Tier-2 edge-band-bias.")
    parser.add_argument("--config", default="configs/system.yaml")
    parser.add_argument("--out", default="results/tier2_third_residual_scenarios")
    args = parser.parse_args()
    out = run_tier2_third_residual_scenarios(config_path=args.config, out_dir=args.out)
    print(f"Wrote {out}")
